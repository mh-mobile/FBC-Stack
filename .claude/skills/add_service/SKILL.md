...
- **READMEやブログに「使用技術」「技術スタック」セクションがあれば、それを正とする**。
  作者の分類・掲載ツール・記載順をそのまま尊重し、依存ファイルからの深掘りで
  作者が挙げていないツールを勝手に追加しない（READMEは最後まで読んで見落とさないこと）。
  依存ファイルはバージョンの確定と裏取りに使い、明らかな矛盾があればPR本文で言及する
- 作者による記載が無い場合のみ、依存ファイルから構成する。その際のカテゴリ名は次から選ぶ:
  `フロントエンド` / `バックエンド` / `データベース` / `テスト` / `Linter/Formatter` / `認証` / `外部サービス・API` / `インフラ`（または `デプロイ`）/ `CI/CD`
...
本文（サービス概要）はfrontmatterの直後に書く。**GitHubリポジトリのREADMEにあるサービス概要をそのまま採用する**こと。要約・言い換え・絵文字の追加などの改変はしない（リンク記法など、そのままでは不自然なマークアップを外す程度は可）。READMEに概要が無い場合のみ、ブログ等をもとに2〜4文で簡潔にまとめる。
